...
import { LogIn, AlertCircle, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.message || 'Google Login failed. Please verify your Firebase configuration.');
    } finally {
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-brand-border/60 shadow-card">
        ...
        {/* Firebase / Deployment Status Banner */}
        <div className="mb-5 p-3 rounded-2xl text-xs flex items-center justify-between border bg-slate-50 border-slate-200">
          <div className="flex items-center space-x-2">
            <span className={`w-2 h-2 rounded-full ${isFbActive ? 'bg-green-500' : 'bg-amber-500'}`} />
            <span className="font-semibold text-slate-700">
              Auth Mode: <span className="text-brand-primary font-bold">{isFbActive ? 'Firebase Active' : 'Direct Sign-in'}</span>
            </span>
          </div>
          {isFbActive && (
        ...
        {/* 1-Click Google Sign In (if Firebase configured or available) */}
        {isFbActive && (
          <div className="mb-5">
            <button
              type="button"
              ...
              className="w-full py-2.5 px-4 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold text-xs rounded-xl shadow-2xs transition-all flex items-center justify-center space-x-2.5 disabled:opacity-50"
            >
        ...
        <div className="mb-5 p-3.5 bg-brand-light/60 border border-brand-secondary/30 rounded-2xl flex items-center justify-between">
        ...
        <div className="mt-6 pt-5 border-t border-brand-border/40 text-center text-xs text-brand-muted">
